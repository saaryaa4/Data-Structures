...
public class RUHungry {
    ...
    public void menu(String inputFile) {
        ...
        int length = Integer.parseInt(StdIn.readLine()); //number of categories (6)
        
        categoryVar = new String[length];
        menuVar = new MenuNode[length];

        for(int i = 0; i < length; i++){ //go through categories
            if(i != 0){
                StdIn.readLine();
            }
            categoryVar[i] = StdIn.readLine(); //category (Appetizer)
            int dishes = Integer.parseInt(StdIn.readLine()); //number of dishes (4)
            
            MenuNode prev = new MenuNode(null, null);

            for(int j = 0; j < dishes; j++){ //go through dishes

                if(j != 0){
                    StdIn.readLine();
                }

                String dishName = StdIn.readLine(); //name of dish (Knight Nachos)

                int ids = StdIn.readInt(); //number of ingredient IDs (9)

                int [] stockID = new int[ids]; //stockID array

                for(int k = 0; k < ids; k++){ //go through ingredients
                    stockID[k] = StdIn.readInt();
                }

                Dish tempDish = new Dish(categoryVar[i], dishName, stockID); //new temp dish for each dish

                if(j == 0){
                    prev.setDish(tempDish); //Night Nachos
                } else {
                    MenuNode next = new MenuNode(tempDish, prev);
                    prev = next;
                }
            }
        ...
        for(int i = 0; i < stockVarSize; i++){
            ...
            do{
                if(ptr.getIngredient().getName().equals(ingredientName)){
                    if(prev == null && ptr.getNextStockNode() == null){
                        stockVar[i] = null;
                        return;
                    } else if(ptr.getNextStockNode() == null){
                        prev.setNextStockNode(null);
                        return;
                    } else if(prev == null) {
    ...
    public StockNode findStockNode (int ingredientID) {
        ...
        for(StockNode x = stockVar[index]; x != null; x = x.getNextStockNode()){
            if(ingredientID == x.getIngredient().getID()) {
                return x;
            }
        }
    ...
    public void updateStock (String ingredientName, int ingredientID, int stockAmountToAdd) {
        ...
        if(ingredientName == null){
            found = findStockNode(ingredientID);
        } else if (ingredientID == -1){
            found = findStockNode(ingredientName);
        } else {
            return;
        }

        found.getIngredient().updateStockLevel(stockAmountToAdd);
    }
        ...
        for(int i = 0; i < menuVar.length; i++){
            ...
            do {
                ...
                for(int j = 0; j < arr.length; j++){
                    StockNode toAdd = findStockNode(arr[j]);
                    cost += toAdd.getIngredient().getCost();
                }
    ...
    public void createStockHashTable (String inputFile){
        ...
        // WRITE YOUR CODE HERE
        stockVarSize = Integer.parseInt(StdIn.readLine()); //10
        stockVar = new StockNode[stockVarSize];
        
        while(StdIn.hasNextChar()){
            int id = StdIn.readInt(); //101
            StdIn.readChar(); //space
            String name = StdIn.readLine(); //Lettuce
            Double price  = StdIn.readDouble(); //3.12
            StdIn.readChar(); //space
            int amount = Integer.parseInt(StdIn.readLine()); //25
    ...
    public void addTransactionNode ( TransactionData data ) { // method adds new transactionNode to the end of LL
        ...
        // WRITE YOUR CODE HERE
        TransactionNode transactionData = new TransactionNode(data, null);

        if( transactionVar == null) {
            transactionVar = transactionData;
        } else {
    ...
    public boolean checkDishAvailability ( String dishName, int numberOfDishes ){
        ...
        Dish temp = found.getDish();
        int[] arr = temp.getStockID();

        for(int i = 0; i < arr.length; i++){
            if(findStockNode(arr[i]).getIngredient().getStockLevel() < numberOfDishes){
                return false;
            }
        }
    ...
    public void order ( String dishName, int quantity ){
        ...
        do{
            boolean check = checkDishAvailability(ptr.getDish().getDishName(), quantity);
            ...
            if(check){
                ...
                TransactionData transactionData = new TransactionData("order", ptr.getDish().getDishName(), quantity, profit, true);
                addTransactionNode(transactionData);

                for(int i = 0; i < arr.length; i++){
                    updateStock(null, arr[i], -1*quantity);
                }
                
                return;   
            } else {
                TransactionData transactionData = new TransactionData("order", ptr.getDish().getDishName(), quantity, 0, false);
                addTransactionNode(transactionData);

                if(ptr.getNextMenuNode() != null){
                    ptr = ptr.getNextMenuNode();
                } else {
                    int index = findCategoryIndex(ptr.getDish().getCategory());
                    ptr = menuVar[index];
                }
            }
    ...
    public void donation ( String ingredientName, int quantity ){
        ...
        if(profit() > 50 && ingredient.getIngredient().getStockLevel() >= quantity){
            TransactionData donation = new TransactionData("donation", ingredientName, quantity, 0, true);
            addTransactionNode(donation);
            updateStock(ingredientName, ingredient.getIngredient().getID(), -1*quantity);
        } else {
            TransactionData donation = new TransactionData("donation", ingredientName, quantity, 0, false);
            addTransactionNode(donation);
        }
    }
    ...
    public void restock ( String ingredientName, int quantity ){
        ...
        double cost = ingredient.getIngredient().getCost() * quantity;

        if(cost <= profit()){
            TransactionData restock = new TransactionData("restock", ingredientName, quantity, -1*cost, true);
            updateStock(ingredientName, ingredient.getIngredient().getID(), -1*quantity);
